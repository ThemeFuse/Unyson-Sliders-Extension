...
class FW_Extension_Slider extends FW_Extension {
	...
	public function load_post_edit_options() {
		...
		if ( ! empty( $custom_settings ) ) {
			$selected                                                        = fw_get_db_post_option( $post->ID,
				$this->get_name() . '/selected' );
			$custom_settings_value                                           = fw_get_db_post_option( $post->ID,
	...
	private function get_slider_population_method_options() {
		...
		return fw()->extensions->get( 'population-method' )->get_population_options( $population_method,
			$multimedia_types, $options );
