...
class FW_Extension_Slider extends FW_Extension
{
	...
	public function load_post_edit_options()
	{
		...
		if (is_null($child_extension)) {
			$message = __('This slider was created correctly, but the code implementation was delete from source code.Delete this post or recovery slider implementation', 'fw');
			wp_die($message);
		}

		if (empty($multimedia_types)) {
			$message = __('This slider was created correctly, but the multimedia_types from config.php file was deleted, please set multimedia_types for this slider type.', 'fw');
			wp_die($message);
		}
				...
				'slider-sidebar-metabox' => array(
					...
					'options' => array(
						...
						'title' => array(
							...
							'value' => $title_value,
							'desc' => __( 'Choose a title for your slider only for internal use: Ex: "Homepage".', 'fw' )
						)
					)
				)),
	...
	private function get_sliders_sets_options()
	{
		...
		if (empty($active_sliders)) {
			$message = __('You don\'t have slider extensions, please create at least one extension for properly work', 'fw');
			wp_die($message);
		}
